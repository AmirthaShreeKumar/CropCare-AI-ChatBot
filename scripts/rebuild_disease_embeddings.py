...
from src.disease_rag import initialize_disease_kb, disease_kb_path
...
def rebuild_disease_embeddings():
    base_path = Path(disease_kb_path)
    logger.info("Starting disease embeddings rebuild for %s", base_path)

    if base_path.exists():
        logger.info("Removing existing disease vector store contents at %s", base_path)
    ...
    if documents:
        logger.info("Found %d source files for disease knowledge indexing", len(documents))
    else:
        logger.info("No markdown source files found in %s; falling back to embedded knowledge payload", base_path)

    initialize_disease_kb(force=True)

    write_vector_metadata(
        base_path,
        source="disease_knowledge_base",
        knowledge_type="disease",
        version=1,
        doc_count=len(documents) if documents else None,
    )

    logger.info("Disease embeddings rebuild complete")


if __name__ == "__main__":
    rebuild_disease_embeddings()
